...
%% Predictive Capabilities Test with Original Data Subset

%% Predictive Capabilities Test with Sequence Dimension Adjustment

% Load the trained network and the original data
load('NET_30day_5min_irradiance.mat', 'Net'); % Load trained model
load('30day_5min_irradiance.mat', 'irradiance_data'); % Load original dataset

% Define the portion of data for testing (e.g., last 20% of data)
numSamples = length(irradiance_data);
testPortion = round(0.2 * numSamples);
testData = irradiance_data(end - testPortion - 13 + 1:end); % Ensure enough data for 13-sequence

% Preprocess test data similarly to training data
if strcmpi(opt.dataPreprocessMode, 'Data Standardization')
    testData = DataStandardization(testData);
elseif strcmpi(opt.dataPreprocessMode, 'Data Normalization')
    testData = DataNormalization(testData);
end

% Prepare sequences of feature dimension 13 for prediction
sequenceLength = 13;
numTestSequences = length(testData) - sequenceLength + 1;
predictions = [];

for i = 1:numTestSequences
    testInput = testData(i:i + sequenceLength - 1);
    if strcmpi(opt.learningMethod, 'LSTM')
        prediction = predict(Net, testInput, 'MiniBatchSize', opt.miniBatchSize);
    elseif strcmpi(opt.learningMethod, 'MLP')
...
% De-normalize/De-standardize predictions to original scale
predictions = deNorm(irradiance_data, predictions, opt.dataPreprocessMode);

% Extract actual values corresponding to predictions
actualValues = testData(sequenceLength:end);

% Plot predictions versus actual values for visual comparison
figure;
plot(1:length(actualValues), actualValues, '-b', 'LineWidth', 1.5); hold on;
plot(1:length(predictions), predictions, '-r', 'LineWidth', 1.5);
legend('Actual Values', 'Predictions');
xlabel('Sample Index');
ylabel('Irradiance');
title('Predictive Test: Model Predictions vs Actual Values (Original Data Subset)');
grid on;

% Calculate and display predictive accuracy metrics
MSE = mean((actualValues - predictions).^2);
RMSE = sqrt(MSE);
